...
def calculate_zero_count(images):
    """Calculate the total number of zero pixels in the image(s)."""
    import numpy as np
    if isinstance(images, list):
        total_zero_pixels = 0
        for img in images:
            if torch.is_tensor(img):
                zero_pixels = (img == 0).sum().item()  # ゼロ画素の数を計算
            else:
                zero_pixels = np.sum(img == 0)  # NumPy配列の場合、ゼロ画素の数を計算
            total_zero_pixels += zero_pixels  # ゼロ画素の合計
        return total_zero_pixels  # ゼロ画素の総数を返す
    else:
        # 単一の画像の場合
        if torch.is_tensor(images):
            zero_pixels = (images == 0).sum().item()  # ゼロ画素の数
        else:
            zero_pixels = np.sum(images == 0)  # NumPy配列の場合
        return zero_pixels  # ゼロ画素の総数を返す
...
def paired_random_crop(img_gts, img_lqs, gt_patch_size, scale, gt_path=None):
    ...
    for _ in range(1):
        ...
        zero_count_gt = calculate_zero_count(img_gts_cropped)

        # Check if the current crop has fewer zero pixels in the GT image
        if zero_count_gt < min_zero_count:
            min_zero_count = zero_count_gt
    ...
    return best_crop_gt, best_crop_lq
